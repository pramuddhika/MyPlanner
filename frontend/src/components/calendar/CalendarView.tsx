...
import { setSelectedDate, setCurrentMonth, setCalendarFilters, clearCalendarFilters } from '@/store/slices/uiSlice';
...
export default function CalendarView({ tasks }: CalendarViewProps) {
    ...
    const filteredTasks = useMemo(() => {
        return tasks.filter((task) => {
            if (calendarFilters.statusId && task.status.statusId !== calendarFilters.statusId)
                return false;
            if (
    ...
    const hasActiveFilters = calendarFilters.statusId || calendarFilters.categoryId;
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800/60">
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-1">
                        <Button
                            ...
                            onClick={() =>
                                dispatch(
                                    setCurrentMonth(currentMonthDate.subtract(1, 'month').format('YYYY-MM'))
                                )
                            }
                        ...
                        <Button
                            ...
                            onClick={() =>
                                dispatch(
                                    setCurrentMonth(currentMonthDate.add(1, 'month').format('YYYY-MM'))
                                )
                            }
                            className="text-slate-400 hover:text-white hover:bg-slate-800"
                        >
                ...
                <div className="flex items-center gap-2">
                    ...
                    <Select
                        ...
                        onValueChange={(val) =>
                            dispatch(
                                setCalendarFilters({ statusId: val === 'all' ? null : Number(val) })
                            )
                        }
                    >
                    ...
                    <Select
                        ...
                        onValueChange={(val) =>
                            dispatch(
                                setCalendarFilters({ categoryId: val === 'all' ? null : Number(val) })
                            )
                        }
                    >
                        <SelectTrigger className="w-[130px] h-8 bg-slate-800/50 border-slate-700/50 text-slate-300 text-xs">
                                ...
                                {dayTasks.slice(0, 3).map((task) => (
                                    <div
                                        ...
                                        className={cn(
                                            ...
                                            STATUS_COLORS[task.status.statusName] || 'bg-slate-700/50 text-slate-300'
                                        )}
                                    >
                                        <span
                                            className={cn(
