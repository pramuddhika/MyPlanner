...
export function EditTaskDialog({ open, onOpenChange, task }: Props) {
    ...
    const formik = useFormik({
        initialValues: {
            ...
            isRemainder: task.isRemainder || false,
            remainderTime: task.remainderTime
                ? dayjs(task.remainderTime).format('YYYY-MM-DDTHH:mm')
                : '',
        },
            ...
            <DialogContent className="bg-slate-900 border-slate-800 text-white sm:max-w-lg max-h-[90vh] overflow-y-auto">
                ...
                <form onSubmit={formik.handleSubmit} className="space-y-4 mt-2">
                    ...
                    <div className="flex justify-end gap-3 pt-2">
                        ...
                        >
                            {formik.isSubmitting ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Update Task'}
                        </Button>
                    </div>
                </form>
            </DialogContent>
